   ...
   # Restrict Zone A users to one booking per month
    if zone_code == "A":
        if monthly_booking_count >= 1:
            ...
            return jsonify({"Try another month": "Zone A(East Pune) members can only book once per month."}), 400
        if zone_a_booking_count >= 1:
            return jsonify({"Try another month": "Booking full for Zone A(East Pune) for this month"}), 400
    
    elif zone_code == "B":
        if monthly_booking_count >= 2:  # Individual restriction
            print("Zone B user allow only 2 booking once per month")
            return jsonify({"Try another month": "Zone B(Rest of Pune) members can only book twice per month."}), 400
        if zone_b_booking_count >= 2:  # Collective restriction
            return jsonify({"Try another month": "Booking full for Zone B(Rest of Pune) for this month"}), 400

    elif zone_code == "C":
        if monthly_booking_count >= 2:  # Individual restriction
            print("Zone C user allow only 2 booking once per month")
            return jsonify({"Try another month": "Zone C(PCMC) members can only book twice per month."}), 400
        if zone_c_booking_count >= 2:  # Collective restriction
            return jsonify({"Try another month": "Booking full for Zone C(PCMC) for this month"}), 400
    ...
    if existing_booking_on_saturday:
        return jsonify({"Try another saturday": "Upasana is booked for this Saturday."}), 400
